package pages

import (
	"fmt"

	"github.com/charmbracelet/lipgloss"
	"github.com/thelastvideostore/internal/models"
	"github.com/thelastvideostore/tui/styles"
)

type SnackBarOrderMsg struct{ ItemID string }

type SnackBarMenuModel struct {
	Items    []models.SnackBarItem
	Selected int
	Balance  float64
	Status   string
	Error    string
}

func NewSnackBarMenuModel(balance float64) *SnackBarMenuModel {
	return &SnackBarMenuModel{Selected: -1, Balance: balance}
}

func (m *SnackBarMenuModel) SetItems(items []models.SnackBarItem) {
	m.Items = items
	if len(items) > 0 && m.Selected < 0 {
		m.Selected = 0
	}
	if len(items) == 0 {
		m.Selected = -1
	}
}

func (m *SnackBarMenuModel) MoveUp() {
	if len(m.Items) == 0 {
		return
	}
	m.Selected--
	if m.Selected < 0 {
		m.Selected = len(m.Items) - 1
	}
}

func (m *SnackBarMenuModel) MoveDown() {
	if len(m.Items) == 0 {
		return
	}
	m.Selected++
	if m.Selected >= len(m.Items) {
		m.Selected = 0
	}
}

func (m *SnackBarMenuModel) SelectedItem() *models.SnackBarItem {
	if m.Selected >= 0 && m.Selected < len(m.Items) {
		return &m.Items[m.Selected]
	}
	return nil
}

func (m *SnackBarMenuModel) View(w, h int) string {
	title := styles.HeadingStyle.Width(w).Align(lipgloss.Center).Render("🍿 SNACK BAR")
	balance := fmt.Sprintf("Balance: $%.2f", m.Balance)

	var rows []string
	rows = append(rows, styles.TextStyle.Render(balance), "")

	if len(m.Items) == 0 {
		rows = append(rows, styles.DimTextStyle.Render("Loading snack bar menu..."))
	}

	for i, item := range m.Items {
		prefix := "  "
		st := styles.TextStyle
		if i == m.Selected {
			prefix = styles.HighlightStyle.Render("▸ ")
			st = styles.HighlightStyle
		}

		stock := styles.SuccessTextStyle.Render(fmt.Sprintf("%d in stock", item.Stock))
		if item.Stock <= 0 {
			stock = styles.ErrorTextStyle.Render("out of stock")
		}

		affordable := ""
		if item.Stock > 0 && m.Balance < item.Price {
			affordable = styles.ErrorTextStyle.Render(fmt.Sprintf(" (need $%.2f more)", item.Price-m.Balance))
		}

		line := fmt.Sprintf("%s %s %-26s $%5.2f  %s%s", prefix, item.Emoji, item.Name, item.Price, stock, affordable)
		rows = append(rows, st.Render(line))
		rows = append(rows, styles.DimTextStyle.Render("    "+item.Description))
	}

	content := lipgloss.JoinVertical(lipgloss.Left, append([]string{title}, rows...)...)
	if m.Status != "" {
		content += "\n" + styles.SuccessTextStyle.Render(m.Status)
	}
	if m.Error != "" {
		content += "\n" + styles.ErrorTextStyle.Render(m.Error)
	}

	return content
}
